...
class OrderCreateView(APIView):
    ...
    def post(self, request):
        ...
        if coupon_code:
            try:
                coupon = Coupon.objects.get(code=coupon_code, is_active=True)
                if hasattr(coupon, 'is_valid') and not coupon.is_valid():
                    return Response(
                        {'error': f'Coupon "{coupon_code}" is expired or fully used.'},
                        status=400
                    )
                discount = coupon.calculate_discount(subtotal)
            except Coupon.DoesNotExist:
...
class AssignMapOnPurchase(APIView):
    ...
    def post(self, request):
        ...
        if not has_tshirt:
            return Response({'error': 'No T-shirt in order'}, status=400)
        ...
        if not released_maps.exists():
            return Response({'error': 'No active maps available'}, status=400)
        ...
        progress, _ = CustomerMapProgress.objects.get_or_create(customer=request.user)
        progress.total_maps_collected += 1
        progress.unique_maps_collected = CustomerMapClaim.objects.filter(
            customer=request.user
        ).values('map').distinct().count()
...
class ClaimMapReward(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'error': 'Map not found or already claimed'}, status=404)

        now          = timezone.now()
        treasure_map = claim.map

        if now < treasure_map.valid_from:
            return Response({'error': 'Map not yet valid', 'valid_from': treasure_map.valid_from}, status=400)

        if now > treasure_map.valid_until:
            claim.status = 'expired'
            claim.save(update_fields=['status'])
            return Response({'error': 'Map expired', 'valid_until': treasure_map.valid_until}, status=400)

        try:
            progress = CustomerMapProgress.objects.get(customer=request.user)
        except CustomerMapProgress.DoesNotExist:
            return Response({'error': 'No map progress found.'}, status=400)
...
class GetActiveMaps(APIView):
    ...
    def get(self, request):
        maps = TreasureMap.objects.filter(is_released=True).order_by('map_type')
