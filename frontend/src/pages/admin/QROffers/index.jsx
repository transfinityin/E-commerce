...
export default function QROffers() {
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...formData,
      fixed_discount:
        formData.reveal_type === 'fixed' ? formData.fixed_discount : '',
    }
  ...
  const downloadQR = (imageUrl, title) => {
    ...
    link.href = imageUrl
    link.download = `qr-${String(title || 'offer')
      .replace(/\s+/g, '-')
      .toLowerCase()}.png`
    link.click()
  }
  ...
  const activeOffers = offers.filter(
    (offer) => offer.is_active && offer.is_valid_now
  ).length
      ...
      <div className="min-h-screen bg-black pt-[76px] sm:pt-[88px] lg:pt-[96px] overflow-x-hidden">
        <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className="h-32 skeleton-dark border border-gold/10" />
            ))}
          </div>

          <div className="flex flex-col gap-3">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className="w-full h-20 skeleton-dark border border-gold/10" />
            ))}
          </div>
        </main>
      </div>
        ...
        <section className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5 mb-7 sm:mb-9">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="btn-outline inline-flex items-center justify-center gap-2 w-full sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {logsLoading ? (
                <>
            ...
            <div className="flex items-center justify-between gap-4 mb-6">
              <div>
                ...
                <h2 className="font-display text-lg sm:text-xl text-white tracking-[0.12em]">
                  {editingOffer ? 'EDIT QR OFFER' : 'CREATE QR OFFER'}
                </h2>
              </div>
                ...
                <Field label="Reveal Type">
                  <select
                    ...
                    onChange={(event) =>
                      setFormData((prev) => ({
                        ...prev,
                        reveal_type: event.target.value,
                      }))
                    }
                    className={inputClass}
                  >
                ...
                <Field label="Max Scans">
                  <input
                    ...
                    onChange={(event) =>
                      setFormData((prev) => ({
                        ...prev,
                        max_scans: parseInt(event.target.value, 10) || 1,
                      }))
                    }
                    min="1"
                    className={inputClass}
                  ...
                  <Field label="Fixed Discount">
                    <input
                      ...
                      onChange={(event) =>
                        setFormData((prev) => ({
                          ...prev,
                          fixed_discount: event.target.value,
                        }))
                      }
                      placeholder="e.g. 25"
                      min="1"
                    ...
                    <Field label="Min Discount">
                      <input
                        ...
                        onChange={(event) =>
                          setFormData((prev) => ({
                            ...prev,
                            min_discount: parseFloat(event.target.value) || 1,
                          }))
                        }
                        min="1"
                        max="99"
                    ...
                    <Field label="Max Discount">
                      <input
                        ...
                        onChange={(event) =>
                          setFormData((prev) => ({
                            ...prev,
                            max_discount: parseFloat(event.target.value) || 1,
                          }))
                        }
                        min="1"
                        max="99"
                ...
                <Field label="Coupon Expiry Minutes">
                  <input
                    ...
                    onChange={(event) =>
                      setFormData((prev) => ({
                        ...prev,
                        scan_expiry_minutes: parseInt(event.target.value, 10) || 5,
                      }))
                    }
                    min="5"
                    className={inputClass}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gold/10 bg-black">
                        {['Offer', 'Discount', 'Coupon', 'Used', 'Scanned At'].map(
                          (heading) => (
                    ...
                    <tbody>
                      {scanLogs.map((log) => (
                        ...
                        >
                          <td className="text-white font-mono px-5 py-4">
                            {log.qr_offer_title}
                          </td>
                          <td className="font-mono text-gold px-5 py-4">
                            {log.discount_revealed}%
                          </td>
                          <td className="font-mono text-white px-5 py-4">
                            {log.coupon_code}
                          </td>
                          ...
                          <td className="text-muted text-xs font-mono px-5 py-4">
                            {log.scanned_at
                              ? new Date(log.scanned_at).toLocaleDateString('en-IN')
                              : 'N/A'}
                          </td>
                        </tr>
                      ))}
            ...
            <section className="hidden lg:block bg-[#0A0A0A] border border-gold/15 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gold/10 bg-black">
                      {['QR Offer', 'Type', 'Discount', 'Scans', 'Status', 'Actions'].map(
                        (heading) => (
                          <TableHead key={heading}>{heading}</TableHead>
                        )
                      )}
                  ...
                  <tbody>
                    {filteredOffers.map((offer) => (
                      ...
                      >
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] text-muted font-mono mt-1 truncate max-w-[260px]">
                                {offer.qr_code_id}
                              </p>
                            </div>
                          </div>
                        ...
                        <td className="px-5 py-4">
                          <TypeBadge type={offer.reveal_type} />
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1.5 text-sm text-muted font-mono">
                            <ScanLine size={14} />
                            <span>
                              {offer.scan_count || 0} / {offer.max_scans}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            <ActionButton onClick={() => handleEdit(offer)} label="Edit">
                              <Edit3 size={14} />
                            </ActionButton>
                            ...
                            <button
                              ...
                              className="w-9 h-9 bg-red-400/10 border border-red-400/20 flex items-center justify-center text-red-400 hover:bg-red-400 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                              title="Delete"
                            >
                              {deletingId === offer.qr_code_id ? (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-3">
                        ...
                        <MiniBox
                          label="Scans"
                          value={`${offer.scan_count || 0}/${offer.max_scans}`}
                        />
                        <MiniBox
                          label="Valid"
                          value={offer.is_valid_now ? 'Active' : 'Inactive'}
                          gold={offer.is_valid_now}
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-4">
                        ...
                        {offer.qr_image_url && (
                          <button
                            type="button"
                            onClick={() => downloadQR(offer.qr_image_url, offer.title)}
                            className="w-10 h-10 bg-black border border-gold/15 text-gold flex items-center justify-center"
...
function StatCard({ icon: Icon, label, value, danger = false }) {
  return (
    <div className="bg-[#0A0A0A] border border-gold/15 p-4 sm:p-5 hover:border-gold/35 transition-all duration-300">
      ...
      <p className="text-[10px] sm:text-xs text-muted font-mono tracking-wider uppercase mt-1">
        {label}
      </p>
    </div>
  )
...
function MiniBox({ label, value, gold = false }) {
  return (
    <div className="bg-black border border-gold/10 p-2.5">
      <p className="text-[9px] text-muted font-mono tracking-wider uppercase mb-1">
        {label}
      </p>

      <p className={`text-xs font-mono truncate ${gold ? 'text-gold' : 'text-white'}`}>
        {value}
      </p>
    </div>
  )
