...
export default function Checkout() {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      // ✅ FIX: JSON.stringify(undefined) returns undefined (not a string),
      //         so .slice() would crash. Guard it properly.
                      ...
                      <p className="text-[10px] sm:text-xs text-[var(--color-muted)] mt-0.5 sm:mt-1">
                        ...
                        {item.size && (
                          <span className="ml-1.5 sm:ml-2">
                            Size: <strong className="text-[var(--color-text)]">{item.size}</strong>
                          </span>
                        )}
                      </p>
                      <p className="text-[10px] sm:text-xs text-[var(--color-muted-light)] mt-0.5">
